  ...
  return (
    <DashboardLayout>
      ...
      >
        ...
        <Table
          columns={[
            { label: 'User / Role', className: 'text-left' },
            { label: 'Action Type', className: 'text-left' },
            { label: 'Target Patient', className: 'text-left' },
            { label: 'IP Address', className: 'text-left' },
            { label: 'Details', className: 'text-right' }
          ]}
          data={loading ? Array.from({ length: 1 }) : paginatedEvents}
          emptyState={
          ...
          footer={
            filteredEvents.length > 0 ? (
              <div className="p-4" style={{ backgroundColor: 'var(--color-surface-elevated)' }}>
                <Pagination page={page} totalPages={totalPages} totalItems={filteredEvents.length} itemsPerPage={itemsPerPage} onPageChange={setPage} />
              </div>
            ) : undefined
          }
          renderRow={(e, index) => {
              ...
              >
                <td className="px-5 py-4">
                  <div className="flex flex-col pr-2">
                    <span className="font-semibold text-(--color-text) truncate">{event.userName}</span>
                    <span className="text-[10px] uppercase font-bold" style={{ color: 'var(--color-primary)' }}>{event.role}</span>
                  </div>
                </td>
                <td className="px-5 py-4">
                  <span
                    ...
                    style={{
                      backgroundColor: isEmergency ? 'var(--color-error-light)' : event.action === 'APPROVE_CONSENT' ? 'var(--color-success-light)' : 'var(--color-primary-ghost)',
                      color: isEmergency ? 'var(--color-error)' : event.action === 'APPROVE_CONSENT' ? 'var(--color-success)' : 'var(--color-primary)',
                    }}
                  >
                ...
                <td className="px-5 py-4">
                  <div className="flex items-center justify-end">
                    <div className="relative group">
                      <button
                        onClick={() => setSelectedMetadata(event.metadata)}
        ...
        {selectedMetadata && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
            ...
            >
              ...
              <div className="rounded-xl p-4 overflow-x-auto max-h-75 border" style={{ backgroundColor: 'var(--color-surface-elevated)', borderColor: 'var(--color-border)' }}>
